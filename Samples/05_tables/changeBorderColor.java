...
public class changeBorderColor {
    public static void main(String[] args) {
      // Create a new PDF document
        ...
        // Define the data for the table/grid
        String[] data = {
        ...
        // Add rows to the grid based on the number of data elements
        ...
        // Set the width of the first column to 120 units
        grid.getColumns().get(0).setWidth(120);

        // Set the width of the second column to 120 units
        grid.getColumns().get(1).setWidth(120);

        // Set the width of the third column to 120 units
        grid.getColumns().get(2).setWidth(120);

        // Set the width of the fourth column to 50 units
        grid.getColumns().get(3).setWidth(50);

        // Set the width of the fifth column to 60 units
        grid.getColumns().get(4).setWidth(60);

        // Set the height of all rows in the grid
        ...
        for (int r = 0; r < data.length; r++) {
            // Split each data row by the semicolon separator
            String[] rowData = data[r].split("[;]", -1);

            // Assign each value to the corresponding cell in the grid
            for (int c = 0; c < rowData.length; c++) {
                grid.getRows().get(r).getCells().get(c).setValue(rowData[c]);
            }
        }

        // Set the font style of the first row (header)
